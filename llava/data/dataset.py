...
def replace_unicode_with_hash(name: str) -> str:
    new_name = ""
    for ch in name:
        if ord(ch) < 128:
            new_name += ch
        else:
            new_name += f"#U{ord(ch):04x}"
    return new_name
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        self.data_args = data_args
        self.image_folder = image_folder
        self.label_balance = data_args.label_balance
        random.shuffle(list_data_dict)
        train_ratio = getattr(data_args, "train_ratio", 1.0)  # extra parameters
        self.list_data_dict = list_data_dict[0 : int(train_ratio * len(list_data_dict))]
    def __len__(self):
        return len(self.list_data_dict)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            image_file = self.list_data_dict[i]["image"]
            image_file = replace_unicode_with_hash(image_file)
...
class LazyVLNCEDataset(Dataset):  # TODO can write
    """Dataset for supervised fine-tuning."""

    def __init__(
        self,
        data_path: str,  # annotations.json list of episode info
        image_folder: str,  # train folders of frames
        tokenizer: transformers.PreTrainedTokenizer,
        data_args: DataArguments,
        training_args: TrainingArguments,
    ):
    ...
    def __len__(self):
        return len(self.list_data_dict)  # split trajectory

    @property
    def lengths(self):
        length_list = []
        for sample in self.list_data_dict:
            img_tokens = 128 if "image" in sample else 0  # 0
            length_list.append(sum(len(conv["value"].split()) for conv in sample["conversations"]) + img_tokens)
        return length_list  # list of len (conv token = image token + text token)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        sources = self.list_data_dict[i]
        if isinstance(i, int):
            sources = [sources]  # one sample
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
        if ("frames" in sources[0]) and ("video_id" in sources[0]):
            num_video_frames = self.data_args.num_video_frames
            frames = sources[0]["frames"]  # list of image path frame_xx.png ['914/frame_0.jpg']
            video_folder = self.image_folder  # image folder path
            video_paths = [os.path.join(video_folder, frame) for frame in frames]  # absoluate path
            # 保留最新一帧(当前帧)以及过去的t-1帧等间距采样 凑齐 num_video_frames 这样好吗
            # 如果不足num_video_frames 就补齐空白帧呢
            images, video_loading_succeed = self._load_video(video_paths, num_video_frames, self.data_args)
            num_frames_loaded_successfully = len(images)
            # resize and stack   num_video_frames 3 H W
            image_tensor = torch.stack([process_image(image, self.data_args, None) for image in images])
            ...
            image_tokens = "<image>\n" * (num_frames_loaded_successfully - 1)  # number of historical frames
            question = (
            ...
            )  # TODO change navigation prompt certain? action label to text action mapping
            ...
            sources = [conversation]  # construct conversation with multi-modal prompt and action answer
        else:
            raise ValueError(f"Unknown data type: {sources[0]}")
        # read time 2025/8/21 17:13
        data_dict = preprocess(
            sources,  # list of list of dict
            self.tokenizer,
            has_image=(
                "image" in self.list_data_dict[i]
                or "video" in self.list_data_dict[i]
        ...
        )  # {"input_ids": inputs, "labels": labels} ignore except assistant answer
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0])

        if ("video" in self.list_data_dict[i]) or ("video_id" in self.list_data_dict[i]):
            data_dict["image"] = image_tensor  # shape = num_video_frames 3 H W
            if not video_loading_succeed:
                data_dict["labels"][:] = IGNORE_INDEX
        else:
            data_dict["image"] = None
        return data_dict  # keys = ['input_ids','labels', '']


class LazyUpdateTripleMergeAirVLNDataset(Dataset):  # TODO can write
    """Dataset for supervised fine-tuning."""
    ...
    def __init__(
        ...
        try:
            ...
            with open(data_path) as fp:
                list_data_dict = [json.loads(q) for q in fp]
        instruction_path = "Dataset/AerialVLN-Dataset/data/aerialvln-s/train_episode2instruction.json" # episode2instruction
        weight_path = "Dataset/AerialVLN-Dataset/data/aerialvln-s/train_action_prob_weight.json" # reweight 
        ids_merge_path = "Dataset/AerialVLN-Dataset/data/aerialvln-s/train_episodes2idx_merge.json" # episode2idx_merge
        with open(instruction_path, "r") as f:
            episode2instruciton = json.load(f)
        self.episode2instruction = episode2instruciton
        with open(weight_path, "r") as f:
            action_prob_weight = json.load(f)
        self.action_prob_weight = action_prob_weight
        with open(ids_merge_path, "r") as f:  # ids after action merge
            ids_merge = json.load(f)
        self.ids_merge = ids_merge
        random.shuffle(list_data_dict) # tripe-samples
        train_ratio = getattr(data_args, "train_ratio", 1.0)  # extra parameters
        self.list_data_dict = list_data_dict[0 : int(train_ratio * len(list_data_dict))]
        self.tokenizer = tokenizer
        self.data_args = data_args
        self.image_folder = image_folder
        self.stop_prob = getattr(data_args, "prob_stop", 0.0)  # extra parameters
        self.idx_type = data_args.idx_type  # past and curren rgb timestep is default or idx of keyframes # default or merge
        # for action label balance
        self.label_balance = data_args.label_balance
        self.label_balance_type = data_args.label_balance_type
        # for memory bank and temporal aggregation
        self.hisotry_queue_len = data_args.hisotry_queue_len
        self.temporal_aggregation_frames = data_args.temporal_aggregation_frames # #frames after temporal aggregation
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            sources = [sources]  # one sample
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
        # make frame path historical & current image
        # TODO maybe add specific history window length?
        num_video_frames = self.data_args.num_video_frames
        video_folder = self.image_folder
        episode_id = sources[0]["episode"]
        idx_merge = self.ids_merge[episode_id]
        
        if random.random() < self.stop_prob:
            answer = "The next action is stop. "
            timestep = idx_merge[-1] # last frame idx
        else:
            timestep = sources[0]["timestep"] # keyframe idx
            answer = sources[0]["action"]  # current action

        if self.label_balance and self.label_balance_type == "v0":
            sample_weight = 1
        elif self.label_balance and self.label_balance_type == "v1": # smooth freq  (mean_prob / prob)^α
            sample_weight = self.action_prob_weight[answer]["norm_weight"] ** 0.5 # freq * 1/sqrt(freq)

        if "stop" in answer:  
            answer = "I think I should stop because I have finished the instruction."
              
        if self.idx_type == "default":  # 0,1,2,3,4,5,6,7 
            frames = [f"{episode_id}/rgb/frame_{t:03d}.jpg" for t in range(timestep + 1)]
        elif self.idx_type == "merge":  # 0,1,7 keyframe idx after merge
            frames = [f"{episode_id}/rgb/frame_{t:03d}.jpg" for t in idx_merge if t <= timestep]
    
        # TODO memory bank history len
        video_paths = [os.path.join(video_folder, frame) for frame in frames[-self.hisotry_queue_len :]]
        images, video_loading_succeed = self._load_video(video_paths, num_video_frames, self.data_args)
        num_frames_loaded_successfully = len(images)
        # resize and stack   num_video_frames 3 H W
        image_tensor = torch.stack([process_image(image, self.data_args, None) for image in images])
        # TODO: Remove extra spaces before punctuation
        instruction = self.episode2instruction[episode_id].replace("\r\n", " ").replace("\n", " ")
        instruction = re.sub(r"(?<=\.\s)([a-z])", lambda x: x.group().upper(), instruction.capitalize())
        instruction = re.sub(r"\s+\.", ".", instruction)

        image_tokens = "<image>\n" * (self.temporal_aggregation_frames - 1) # first frame should one image placeholder
        question = (
            f"Imagine you are a robot programmed for navigation tasks. You have been given a video "
            f"of historical observations {image_tokens}, and the current observation <image>.\n"
        ...
        data_dict["image"] = image_tensor # T C 448 448 
        if not video_loading_succeed:
            data_dict["labels"][:] = IGNORE_INDEX
...
class LazyUpdateTripleMergeAirVLNSubTrajectorySummaryDataset(Dataset):  # TODO can write
    """Dataset for supervised fine-tuning."""
    ...
    def __init__(
        ...
        try:
            with open(data_path) as fp:
                list_data_dict = json.load(fp)# subgoal_pointer_list.json
        except:
            with open(data_path) as fp: 
                list_data_dict = [json.loads(q) for q in fp]
        
        segments_path = 'Dataset/AerialVLN-Dataset/data/aerialvln-s/labeled_segments.json' # TODO need change
        ...
        self.data_args = data_args
        self.image_folder = image_folder
        self.idx_type = data_args.idx_type  # past and curren rgb timestep is default or idx of keyframes
        self.hisotry_queue_len = data_args.hisotry_queue_len
        self.temporal_aggregation_frames = data_args.temporal_aggregation_frames # #frames after temporal aggregation
        self.label_balance =  data_args.label_balance    
    def __len__(self):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        episode_id = self.list_data_dict[i]["episode_id"]
        seg_id = self.list_data_dict[i]["seg_id"] -1
        sources = self.segments[episode_id][seg_id]
        if isinstance(i, int):
            sources = [sources]  # one sample
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
        num_video_frames = self.data_args.num_video_frames
        video_folder = self.image_folder
        idx_merge = sources[0]['keyimstep']
        # step 1, sample frames from whole trajectory
        if self.idx_type == "default":
            frames = [f"{episode_id}/rgb/frame_{t:03d}.jpg" for t in range(idx_merge[-1] + 1)]
        else:
            frames = [f"{episode_id}/rgb/frame_{t:03d}.jpg" for t in idx_merge]
        # next_timestep = idx_merge[len(frames)] if len(frames) < len(idx_merge) else idx_merge[-1]
        video_paths = [os.path.join(video_folder, frame) for frame in frames[-self.hisotry_queue_len :]]
        num_video_frames = self.data_args.num_video_frames
        images, video_loading_succeed = self._load_video(video_paths, num_video_frames, self.data_args)
        num_frames_loaded_successfully = len(images)
        # resize and stack   num_video_frames 3 H W
        image_tensor = torch.stack([process_image(image, self.data_args, None) for image in images])
        # downsampl the historical+current frames by temporal_aggregation_rate
        # step 2: get instruction as answer
        instruction = sources[0]["sub_instruction"].replace("\r\n", " ").replace("\n", " ")
        instruction = re.sub(r"(?<=\.\s)([a-z])", lambda x: x.group().upper(), instruction.capitalize())
        instruction = re.sub(r"\s+\.", ".", instruction)
        answer = instruction  # summary of trajectory

        image_tokens = "<image>\n" * (self.temporal_aggregation_frames) # first frame should one image placeholder
        question = (
            f"Imagine you are a robot programmed for navigation tasks. You are provided with captured images sequences "
            f"{image_tokens}. Based on this image sequence, please describe the navigation trajectory of the robot."
        )
        if not video_loading_succeed:
            answer = "Empty video."
...
class LazyOpen3DVQADataset(Dataset):
    """Dataset for supervised fine-tuning.
    """
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
        # image_info
        image_file = self.list_data_dict[i]["image_info"]['image_path']
        image_file = replace_unicode_with_hash(image_file.split("\\")[-1])
        image = process_image(image_file, self.data_args, self.image_folder)
        sources = preprocess_multimodal(copy.deepcopy([e["conversation"] for e in sources]), self.data_args)
...
class DataCollatorForSupervisedDataset:
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        #  data_dict  # keys = ['input_ids','labels', 'image'] image shape 8 3 H W
        input_ids, labels, images = [], [], []
        for instance in instances:
            if not isinstance(instance["input_ids"], list):  # True
            ...
            if instance.get("image") is not None:
                ...
                assert len(cur_image.shape) == 4  # T 3 H W
                ...
                if not isinstance(instance["input_ids"], list):
                    ...
                    # coyo-like datasets
                    images.extend(cur_image.chunk(cur_image.size(0), 0))  # list of image
            else:
                images.append([])
        ...
        # to truncated <image> tokens later. remove this check because temporal downsample 
        temporal_aggregation_rate = instances[0].get("temporal_aggregation_rate",1)
        # check len(images) and len(IMAGE_TOKEN_INDEX) same?
        # for _images, _input_ids in zip(images, input_ids):  # image - input_ids pair
        #     assert (
        #         len(_images) == (_input_ids == IMAGE_TOKEN_INDEX).sum().item() * temporal_aggregation_rate
        ...
        # pading & truncated with model_max_length(4096)
        input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )  # B L
        ...
        for ix in range(len(input_ids)):  # each sample
            cur_images = images[ix]  # T 3 H W
            # num_images = (input_ids[ix] == IMAGE_TOKEN_INDEX).sum().item() * temporal_aggregation_rate
            # cur_images = cur_images[:num_images]  # deleta some <image> tokens get removed after truncation
            if len(cur_images) > 0:
                new_images.append(cur_images)
        
        if len(new_images) > 0:
            batch["images"] = new_images # batchsize num_video C H W 
            # batch["images"] = torch.cat(new_images, dim=0)  # batchsize*num_video
        else:
        ...
        # if instances[0].get("sample_weight") is not None:
        #     batch["sample_weight"] = torch.tensor(
        #         [instance.get("sample_weight", 1.0) for instance in instances], dtype=torch.float
        #     )
...
class DataCollatorForSupervisedDatasetSeqParallel:
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # Sort with the real length of the sequence  适合不同图像的数据
        combined = sorted(
...
def make_supervised_data_module(
    ...
    datasets_mixture.register_datasets_mixtures()  # add dataset

    from .builder import build_dataset

    print(f"Building dataset with mixture: {data_args.data_mixture}")
    ...
    training_args.sample_lens = [len(d) for d in train_dataset.datasets]  # len of each dataset
    # training_args.eval_sample_lens = [len(d) for d in eval_dataset.datasets]

    PROCESS_GROUP_MANAGER = get_pg_manager()

    if PROCESS_GROUP_MANAGER is None:  # True
