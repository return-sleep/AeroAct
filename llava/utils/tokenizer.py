...
def tokenize_conversation_legacy(
    ...
    for turn, message in enumerate(messages):
        role = roles[message["from"]]  # map to roles
        assert role == conv.roles[turn % 2]  #
        if overrides is not None and message["from"] in overrides:  # add sential token
...
def tokenize_conversation(
    messages: Sequence[Dict[str, str]],  # [ {"from": "human", "value": question},{}]
    tokenizer: transformers.PreTrainedTokenizer,
    add_generation_prompt: bool = False,
    overrides: Optional[Dict[str, str]] = None,
    no_system_prompt: bool = False,
    ...
    for m in messages:
        ...
        if overrides is not None and m["from"] in overrides:
            message["content"] = overrides[m["from"]]  # gpt的回答被overrides覆盖
        conversation.append(message)  # [{"user": "human", "content": question},{"user": "assistant", "content": answer} ]
...
def preprocess_conversation(
    conversation: Sequence[Dict[str, str]],  # {"from": "human", "value": question},
    tokenizer: transformers.PreTrainedTokenizer,
    no_system_prompt: bool = False,
) -> Dict[str, Any]:
    ...
    for k in range(template.size(0) - 1):
        if template[k] == tokenizer.sentinel_token_id:
            ...
            if k > 0 and tokenizer.decode(template[k - 1]) == "":
                mask[k - 1] = False
    template = template[mask]  # template 就是“只有上下文（非助理回答）的 token 序列”
    ...
    for k in range(inputs.size(0)):
        if p < template.size(0) and inputs[k] == template[p]:
            ...
            labels[k] = inputs[k]  # 仅 assistant 段落打标 的 labels
