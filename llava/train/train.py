...
def smart_tokenizer_and_embedding_resize(
    ...
    if num_new_tokens > 0:  # initial embedding for new token
...
def train():
    ...
    # save data_args to json file for convinient evaluation
    os.makedirs(training_args.output_dir, exist_ok=True)
    with open(os.path.join(training_args.output_dir, "data_args.json"), "w", encoding="utf-8") as f:
        json.dump(asdict(data_args), f, indent=4, ensure_ascii=False)
    ...
    output_file = os.path.join(training_args.output_dir, "all_args.json")
    ...
    if resume_path:
        ...
        if getattr(config, "resume_path", None) is not None:
            config.resume_path = model_args.model_name_or_path  # a8cheng/navila-siglip-llama3-8b-v1.5-pretrain
    ...
    model = model_cls(
        ...
        model_max_length=training_args.model_max_length,  # 4096
        cache_dir=training_args.cache_dir,
        **bnb_model_from_pretrained_args,
    )  # initial model from model_args.model_name_or_path
    print("finish loading model")
    if not resume_path or training_args.lora_enable:
        if model_args.mlp_path is not None:  # False
    ...
    if model_args.version == "v0":
        ...
        tokenizer.pad_token = tokenizer.unk_token  # unknow token
        if tokenizer.pad_token is None:  # Flase
            smart_tokenizer_and_embedding_resize(
                special_tokens_dict=dict(pad_token="[PAD]"),
                tokenizer=tokenizer,
                model=model.llm,
            )
        # Change conv template version
        if model_args.version in conversation_lib.conv_templates:  # define template
            conversation_lib.default_conversation = conversation_lib.conv_templates[model_args.version]
        else:
            conversation_lib.default_conversation = conversation_lib.conv_templates["vicuna_v1"]

    # kentang-mit@: It will be useful in on-the-fly packing
    model.llm.pad_token_id = tokenizer.pad_token_id  # 0?
    model.llm.config.tokenizer_padding_side = tokenizer.padding_side  # right
    model.llm.config.tokenizer_model_max_length = tokenizer.model_max_length  # 4096
    if training_args.lora_enable:  # for peft model, should use model.base_model
    ...
    if vision_tower is not None:
        ...
        # add time-frame token <t1> <t2> not deploy
        ...
        if model_args.num_time_tokens > 0:  # add <t1><t2> special token
        ...
        model.config.soft_ce_std = model_args.soft_ce_std  # soft cross-entroy
    ...
    if training_args.lora_enable:
        ...
        if training_args.local_rank == 0 or training_args.local_rank == -1:
            model.config.save_pretrained(training_args.output_dir)  # 通常只有 LoRA 适配器参数 不包含基座模型权重
            model.save_pretrained(training_args.output_dir, state_dict=state_dict)  # 不是 LoRA 的可训练模
            torch.save(
                non_lora_state_dict,
