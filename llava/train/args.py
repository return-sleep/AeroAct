...
class DataArguments:
    ...
    # for video training
    num_video_frames: int = 8 # TODO ?
    fps: float = 0.0  # 0.0 means we do not use fps at all. Always sample the same number of frames.
    train_ratio:float = 1.0
    idx_type: str = 'default' # default or merge
    # history samping
    hisotry_queue_len: int = 0 # default 0-current all frames or newest len images
    temporal_aggregation_frames : int = 1 # 4
    K_recent : int = 4
    # for label reweight
    prob_stop:float = 0.0 
    label_balance: bool=False
    label_balance_type: str = 'v0'
