...
def get_model_config(config):
    ...
    # config._name_or_path: /remote-home/xhl/.cache/huggingface/hub/models--a8cheng--navila-siglip-llama3-8b-v1.5-pretrain/snapshots/e9710a222d386dad478e0e1a2712b56e79b88d14
    # print('llava/model/utils.py', config._name_or_path)
    if hasattr(config, "_name_or_path") and len(config._name_or_path) >= 2:
        root_path = config._name_or_path
    else:
        root_path = config.resume_path  # a8cheng/navila-siglip-llama3-8b-v1.5-pretrain
    # print('llava/model/utils.py', root_path)
    # print('llava/model/utils.py', config.resume_path)
    ...
    for key in default_keys:
        cfg = getattr(config, key, None)  # "llm_cfg"
        if isinstance(cfg, dict):  # yes
            try:
                return_list.append(os.path.join(root_path, key[:-4]))  # llm_cfg ¡ú llm, llm  mm_projector  vision_tower
            except:
