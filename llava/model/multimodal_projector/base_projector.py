...
class DownSampleBlock_3(nn.Module):
    def forward(self, x):
        vit_embeds = x
        h = w = int(vit_embeds.shape[1] ** 0.5)
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], h, w, -1)
...
class MultimodalProjector(PreTrainedModel):
    ...
    def __init__(self, mm_projector_cfg: MultimodalProjectorConfig, config: PretrainedConfig):
        ...
        elif mm_projector_type == "mlp_downsample" or mm_projector_type == "mlp_downsample_2x2_fix" :
            self.layers = nn.Sequential(
                DownSampleBlock(), # B H*W D -> B H*W/4 D*4 spatial-to-channel (STC) reshape
                nn.LayerNorm(config.mm_hidden_size * 4),
                nn.Linear(config.mm_hidden_size * 4, config.hidden_size),
                nn.GELU(),
                nn.Linear(config.hidden_size, config.hidden_size),
            )
        elif mm_projector_type == 'mlp_downsample_3x3_fix': # for NVILA-2B
            self.layers = nn.Sequential(
                DownSample3x3BlockFix(), # B H*W D -> B H*W/4 D*4 spatial-to-channel (STC) reshape
