...
class LlavaMetaModel(ABC):
    def init_vlm(self, config: PreTrainedModel = None, *args, **kwargs):
        ...
        if hasattr(self, "llm") or hasattr(self, "vision_tower") or hasattr(self, "mm_projector"):
            ...
            return
        print(config.mm_hidden_size)
        model_dtype = getattr(config, "model_dtype", "torch.float16")
        if not hasattr(config, "model_dtype"):
            warnings.warn("model_dtype not found in config, defaulting to torch.float16.")
            config.model_dtype = model_dtype

        cfgs = get_model_config(config) # path to pretrained weight folder, xxx/llm, xxx/mm_projector, xxx/vision_tower
        if len(cfgs) == 3:
        ...
        self.llm, self.tokenizer = build_llm_and_tokenizer(llm_cfg, config, *args, **kwargs) # from llm_cfg

        self.vision_tower = build_vision_tower(vision_tower_cfg, config) # from vision_tower_cfg

        self.mm_projector = build_mm_projector(mm_projector_cfg, config) #  if config.resume_path from mm_projector_cfg
    ...
    def encode_images(self, images):
        # print('llava/model/llava_arch.py Line 235: encode_images')
        # print(' Raw image shape:', images.shape) # B*N_frames 3 H W [48, 3, 384, 384]
        image_features = self.get_vision_tower()(images) # B*N_frames H*W D
        # print(' Vision tower output shape:', image_features.shape) # [B*T, 729, 1152] for 386  [B*T, 1024, 1152] for 484
        image_features = self.get_mm_projector()(image_features) # # B*N_frames H*W/4 D*4 -> B*N_frames H*W/4 D -> B*N_frames H*W/4 hidden_size [48, 196, 4096] [B*T,121, 4096]
        # for  NVILA-2B  STC 3*3  for NVILA-8B STC 2*2
        # print(' MM projector output shape:', image_features.shape)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list: # for batch samples
            num_frames = [len(image) for image in images]
            images = torch.cat(images, dim=0) # # batch_size x num_video_frames  3  H  W
        elif images.ndim == 5:  # batch_size x seq_len x image_channels
            num_frames = images.shape[1]
            images = images.flatten(0, 1) #(batch_size x seq_len) x 3 x H x W
        else:
            num_frames = images.shape[0] // input_ids.shape[0]
        # Step 1: Encode images TODO merge temporal dimension to fixed num_images
        image_features = self.encode_images(images).to(self.device) # vision_tower -> mm_adapter -> visual tokens
        # print('image_features shape', image_features.shape) #(batch_size x seq_len) x 196 x 4096 
        ...
        # Step 2: convert ids to embedding
        ...
        input_embeds = self.llm.model.embed_tokens(input_ids_copy) #ids -> token embedding (B N E)
        ...
        new_input_embeds = []
        new_labels = []
        # cur_image_idx = 0 # batch_idx x num_images
        # Step 3: insert visual token into input_embedding
        # kentang-mit@: If some part of the model is executed in the loop, the the loop length needs to be a constant.
        
        image_features_split = torch.split(image_features, num_frames, dim=0) # split by batch dimension  N_h*N_w = 121
        for batch_idx, cur_input_ids in enumerate(input_ids):
            cur_input_ids = input_ids[batch_idx] # L E
            num_images = (cur_input_ids == IMAGE_TOKEN_INDEX).sum() #img idx number self.temporal_aggregation_frames
            if num_images == 0: # text only
            ...
            ) # [-1,frame_idx, L] len(frame_idx) = num_video_frames #-200
            cur_input_ids_noim = []
            cur_labels = labels[batch_idx]
            cur_labels_noim = []
            cur_input_embeds_no_im = []
            for i in range(len(image_token_indices) - 1): # split the sentence by image token
                if sp_degree > 1 and i == 0 and sp_rank != 0:  # Handle sequence parallelism
                    ...
                    continue
                cur_input_ids_noim.append(cur_input_ids[image_token_indices[i] + 1 : image_token_indices[i + 1]]) #split input by image_token
                cur_labels_noim.append(cur_labels[image_token_indices[i] + 1 : image_token_indices[i + 1]])
                cur_input_embeds_no_im.append(cur_input_embeds[image_token_indices[i] + 1 : image_token_indices[i + 1]]) #noimage
            # add image token into input_embedding and adjust label
            cur_new_input_embeds = []
            cur_new_labels = []
            image_features_sample = image_features_split[batch_idx] # N H*W D
            image_features_pool = image_features_sample
            
            cur_image_idx = 0 #i-th num_images
            for i in range(num_images + 1):
                cur_new_input_embeds.append(cur_input_embeds_no_im[i]) # i-th segment
                cur_new_labels.append(cur_labels_noim[i]) # # i-th segment
                if i < num_images:
                    # cur_image_features = image_features[cur_image_idx] # feature for i-th image
                    cur_image_features = image_features_pool[cur_image_idx] # feature for i-th image
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features)
                    cur_new_labels.append(
                        torch.full(
                            (cur_image_features.shape[0],), #num_visual_token
                            IGNORE_INDEX,
        ...
        new_input_embeds = torch.stack(new_input_embeds_padded, dim=0) # B max_len E
        ...
        if _labels is None:
            ...
            new_labels = new_labels_padded # (batch_size, max_len)

        if _attention_mask is None:
            attention_mask = None 
        else:
            attention_mask = attention_mask.to(dtype=_attention_mask.dtype) # #  (batch_size, max_len)

        if _position_ids is None:
            position_ids = None #   (batch_size, max_len)

        # We will not use packing here when sequence parallelism is enabled.
        if PROCESS_GROUP_MANAGER is not None:
            return (
                None,
                _position_ids,#  (batch_size, max_len)
                attention_mask,#  (batch_size, max_len)
                past_key_values,
                new_input_embeds, #  (batch_size, max_len, embedding_size)
                new_labels,#  (batch_size, max_len)
            )
