...
def process_image(image_file, data_args, image_folder):
    processor = data_args.image_processor  # data_args.image_processor = vision_tower.image_processor
    ...
    if data_args.image_aspect_ratio == "resize":  # True
        if hasattr(data_args.image_processor, "crop_size"):
            ...
            crop_size = data_args.image_processor.size  # 384
...
def tokenizer_image_token(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, return_tensors=None, lstrip=False):
    ...
    # 在一个列表 X 中的每两个元素之间插入一个分隔符 sep，除了最后一个元素
    # [x_sub1] [seq]  [x_sub2] [seq]... [x_sub3] list of list
    ...
    if lstrip:
        ...
        if len(prompt_chunks) > 0 and len(prompt_chunks[0]) > 0 and prompt_chunks[0][0] == tokenizer.bos_token_id:
            offset = 1
            input_ids.append(prompt_chunks[0][0])  # bos token id

    for chunk_id, x in enumerate(insert_separator(prompt_chunks, [image_token_index] * (offset + 1))):
        if chunk_id == 0 and lstrip:
            input_ids.extend(x)
        else:
            # [bos x_sub1] [img img ]  [bos x_sub2] [img img ]... [bos x_sub3]
            # [x_sub1,img,x_sub2,img,x_sub3]
            input_ids.extend(x[offset:])  # get! skil bos
