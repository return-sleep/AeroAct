from enum import Enum # construct an Enum for default actions
...
class _DefaultAirsimActions(Enum): # Default actions for Airsim
    STOP = 0
    MOVE_FORWARD = 1
    TURN_LEFT = 2
    TURN_RIGHT = 3
...
if __name__ == "__main__":
    # For testing purposes, print the default actions and their values
    for action_name, action_value in AirsimActions._known_actions.items():
        print(f"{action_name}: {action_value}") # STOP: 0
    

    stop_action = AirsimActions.STOP        
    forward_action = AirsimActions['MOVE_FORWARD']  


    step_size = AirsimActionSettings.FORWARD_STEP_SIZE  

    print(f"Stop Action ID: {stop_action}, Forward Action ID: {forward_action}, Step Size: {step_size}")

    for action_name in AirsimActions:
        print(f"{action_name}: {AirsimActions[action_name]}")