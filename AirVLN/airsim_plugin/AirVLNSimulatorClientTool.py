...
class AirVLNSimulatorClientTool:
    ...
    def getImageResponses(self, get_rgb=True, get_depth=True):
        ...
        def _getImages(airsim_client: airsim.VehicleClient, scen_id, get_rgb, get_depth):
                ...
                while True:
                    try:
                        ...
                        if get_rgb:
                            ...
                            if args.run_type not in ['eval']: #图锟斤拷锟斤拷锟斤拷为锟斤拷/锟斤拷取锟斤拷锟斤拷锟斤拷/锟斤拷锟斤拷锟斤拷
            ...
            # plt.imsave('img_rgb.png', img_rgb)

            # img_rgb = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2RGB)

            # plt.imsave('img_rgb_converted.png', img_rgb)

            # plt.imsave('img_depth.png', img_depth.squeeze(), cmap='gray')
...
if __name__ == '__main__':
    ...
    machines_info_xxx = [
        {
            ...
            'SOCKET_PORT': 30000,
            'MAX_SCENE_NUM': 1, # simulation environment
            'open_scenes': [1], # envs id
        },
    ]
    # machines_info_xxx = [
    #     {
    ...
    #     },
    # ]
    # 锟剿口凤拷锟戒、settings 写锟诫、AirVLN.sh 锟斤拷锟斤拷锟斤拷图锟斤拷涌诔锟绞硷拷锟?
    ...
    while True:
        time_1 = time.time()
        responses = tool.getImageResponses() # 锟缴硷拷锟斤拷锟斤拷锟斤拷锟?
        time_2 = time.time()
        print(
