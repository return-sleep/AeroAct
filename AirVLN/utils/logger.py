...
class AirsimLogger(logging.Logger):
    def __init__(
        self,
        name,
        level, # logging.DEBUG, logging.INFO or else
        filename=None, # name of the log file
        filemode="a",
        stream=None,
        format_str=None,
        dateformat=None,
    ...
    def add_filehandler(self, log_filename): # add a new file handler
...
logger = AirsimLogger(
    name="airsim",
    level=logging.INFO if int(os.environ.get("RANK", -1)) in [-1, 0] else logging.WARNING, # log only on rank 0
